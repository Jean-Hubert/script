...
-- Généré le :  Ven 25 Mai 2018 à 11:13
...
--
-- Doublure de structure pour la vue `pathologies`
--
...
--
-- Doublure de structure pour la vue `pathologies_subquery`
--
CREATE TABLE `pathologies_subquery` (
`idP` int(11)
,`desc` varchar(50)
,`type` varchar(10)
,`element` varchar(1)
...
--
-- Structure de la vue `pathologies`
--
DROP TABLE IF EXISTS `pathologies`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `pathologies` AS select `tbl`.`idP` AS `idP`,`tbl`.`desc` AS `desc`,`tbl`.`type` AS `type`,`tbl`.`element` AS `idMer`,`tbl`.`code` AS `codeMer`,`tbl`.`nom` AS `nomMer`,`tbl`.`yin` AS `yinMer`,group_concat(`tbl`.`motcles` separator ',') AS `motcles`,group_concat(`tbl`.`symptomes` separator ',') AS `symptomes` from `pathologies_subquery` `tbl` group by `tbl`.`idP`,`tbl`.`desc`,`tbl`.`type`,`tbl`.`element`,`tbl`.`nom`,`tbl`.`yin`;

-- --------------------------------------------------------

--
-- Structure de la vue `pathologies_subquery`
--
/*DROP TABLE IF EXISTS `pathologies_subquery`;*/


CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `pathologies_subquery` AS select `patho`.`idP` AS `idP`,`patho`.`desc` AS `desc`,`patho`.`type` AS `type`,`meridien`.`element` AS `element`,`meridien`.`nom` AS `nom`,`meridien`.`code` AS `code`,`meridien`.`yin` AS `yin`,group_concat(`keywords`.`name` separator ',') AS `motcles`,`symptome`.`desc` AS `symptomes` from (((((`symptPatho` join `keySympt` on((`symptPatho`.`idS` = `keySympt`.`idS`))) join `keywords` on((`keySympt`.`idK` = `keywords`.`idK`))) join `patho` on((`patho`.`idP` = `symptPatho`.`idP`))) join `symptome` on((`symptome`.`idS` = `symptPatho`.`idS`))) join `meridien` on((`meridien`.`code` = `patho`.`mer`))) group by `patho`.`idP`,`patho`.`desc`,`symptome`.`desc`,`patho`.`type`,`meridien`.`nom`,`meridien`.`element`,`meridien`.`code`,`meridien`.`yin`,`symptome`.`desc`;
...
ALTER TABLE `keySympt`
  ...
  ADD KEY `keySympt_symptome_idS_fk` (`idS`);
...
ALTER TABLE `symptPatho`
  ...
  ADD KEY `symptPatho_patho_idP_fk` (`idP`);
...
--
-- Contraintes pour la table `keySympt`
--
ALTER TABLE `keySympt`
  ADD CONSTRAINT `keySympt_keywords_idK_fk` FOREIGN KEY (`idK`) REFERENCES `keywords` (`idK`),
  ADD CONSTRAINT `keySympt_symptome_idS_fk` FOREIGN KEY (`idS`) REFERENCES `symptome` (`idS`);

--
-- Contraintes pour la table `patho`
--
ALTER TABLE `patho`
  ADD CONSTRAINT `patho_meridien_code_fk` FOREIGN KEY (`mer`) REFERENCES `meridien` (`code`);

--
-- Contraintes pour la table `symptPatho`
--
ALTER TABLE `symptPatho`
  ADD CONSTRAINT `symptPatho_patho_idP_fk` FOREIGN KEY (`idP`) REFERENCES `patho` (`idP`),
  ADD CONSTRAINT `symptPatho_symptome_idS_fk` FOREIGN KEY (`idS`) REFERENCES `symptome` (`idS`);
